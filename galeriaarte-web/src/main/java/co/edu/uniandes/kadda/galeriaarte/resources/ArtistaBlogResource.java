...
public class ArtistaBlogResource {
    ...
    public BlogDetailDTO getBlog(@PathParam("id") Long idartista, @PathParam("idBlog") Long idBlog) throws BusinessLogicException {
        ...
        if (ent == null) {
            throw new BusinessLogicException("No existe el proveedor con id " + idartista);
        }
        BlogDetailDTO fbDetail = null;
        for (BlogEntity fb : ent.getBlogs()) {
            if (fb.getId().equals(idBlog)) {
                fbDetail = (new BlogDetailDTO(fb));
            }
        }
        if (fbDetail == null) {
            throw new BusinessLogicException("No existe la obra con id " + idBlog + " del proveedor con id " + idartista);
        }
        return fbDetail;
    }
