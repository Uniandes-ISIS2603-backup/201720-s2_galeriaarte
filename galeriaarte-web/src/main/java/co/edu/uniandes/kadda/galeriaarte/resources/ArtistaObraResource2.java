...
public class ArtistaObraResource2
{
@Inject
 ...
 @GET 
    ...
    public ObraDetailDTO getObra ( @PathParam("id") Long idProveedor, @PathParam("idObra") Long idFeedBack) throws BusinessLogicException
    {
        ...
        for (ObraEntity fb : ent.getObras())
            if (fb.getId().equals(idFeedBack)) fbDetail = (new ObraDetailDTO(fb));
        if (fbDetail == null) throw new BusinessLogicException("No existe la obra con id " + idFeedBack + " del proveedor con id " + idProveedor);
        return fbDetail;
    }
